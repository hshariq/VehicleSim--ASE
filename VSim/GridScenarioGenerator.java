...
/**
* 
* Generates random traffic scenarios with a specific number of {@link Vehicle} instances. 
* It also supports the generation of scenarios with an upper-bound on the number of grid squares their paths can intersect. 
* 
* @author MEZZO Inc. 
* 
*/
...
public class GridScenarioGenerator{
    ...
    /**
     * Creates the vehicles for the scenario, with random start and destination points, as well as random speeds.
    ...
    public Vehicle[] generateScenario(int nVehicles) {
        ...
        for(int i=0;i<nVehicles;i++){
            ...
            scenario[i]= new Vehicle(startX, startY, destX, destY, timePerSq, new NorthFirst(), sim);
    ...
    /**
     * Creates the vehicles for the scenario, with random start and destination points, as well as random speeds.
     * It ensures the vehicle's paths intersect over at most <i>maxIntersectionLength</i> grid squares.
     * 
     * 
     * @param nVehicles the number of vehicles in the generated scenario
     * @param maxIntersectionLength the maximum allowed path intersection length in the generated scenario
     * 
     * @return an array containing the generated vehicles. 
     */
    ...
    public Vehicle[] generateScenario(int nVehicles, int maxIntersectionLength) {
        ...
        while(i<nVehicles){
            ...
            List<int[]> path = new NorthFirst().findNorthFirstPath(grid,startX, startY, destX, destY);
            ...
            if(i==0)  {
                ...
                boolean isGood=true;
                for(int j=0; j<i;j++){
                    // check if intersection between paths is smaller or equal the desired upper-bound
                    isGood = testPath(scenario[j].getPath(), path, maxIntersectionLength);
                    if(!isGood) {
                        // path intersection is larger than desired upper bound, so discard it and try again
                        break;
                    }
                }

                if(isGood) {
                    // path intersection is not larger than desired upper bound, so add new vehicle to the scenario
                    scenario[i] = new Vehicle(startX, startY, destX, destY, timePerSq, path, sim);
                    i++;
                }
            }
